...
# 创建窗口
root = tkinter.Tk()
root.title("BASIC Editor for LASER-310 v1.0.7 by odorajbotoj")
root.resizable(0, 0)
...
basicObj = {"fileVer": fileVer, "lineNum": 0, "lines": []}
currentLineObj = {"lineNum": 0, "blocks": []}

# 编辑区
editFrame = tkinter.LabelFrame(root, text="编辑")

basicFrame = tkinter.LabelFrame(editFrame, text="BASIC")
basicTextArea = tkinter.scrolledtext.ScrolledText(
    basicFrame, height=20, width=50, state="disabled"
)
...
def updateText():
    ...
    txt += (
        str(currentLineObj["lineNum"]) + " " + "".join(currentLineObj["blocks"]) + "\n"
    )
...
def insertREM():
    if checkEntry():
        ...
        if len('{} REM "{}"'.format(basicObj["lineNum"], txt)) > 60:
            ...
            return
        basicObj["lines"].append(
            {"lineNum": basicObj["lineNum"], "blocks": ["REM", " ", txt]}
        )
        updateText()
        mainEntry.set("")
...
tkinter.Button(editFrame, text="SPACE", command=insertSPACE).grid(
    row=2, column=1, columnspan=2
)
tkinter.Button(editFrame, text="ENTER", command=insertENTER).grid(
    row=2, column=3, columnspan=2
)
tkinter.Button(editFrame, text="BACKSPACE", command=backspace).grid(
...
for k1 in basicDicts:
    ...
    for i in range(len(ks)):
        tkinter.Button(fr, text=ks[i], command=lambda arg=ks[i]: buttonClick(arg)).grid(
            row=i // 6, column=i % 6
        )
...
def openFile():
    ...
    filename = tkinter.filedialog.askopenfilename(
        title="打开", initialfile="basic_code.json", filetypes=[("JSON", ".json")]
    )
    # DEBUG
    # print(filename)
    if len(filename) == 0:
...
def exportWAV():
    global basicObj
    basicName = tkinter.simpledialog.askstring(
        "输入程序名", "请输入程序名\n15个以内合法字符"
    )
    if basicName == None:
        return
    ...
    # 问开始地址
    startAddr = tkinter.simpledialog.askinteger(
        title="输入起始地址",
        prompt="请输入程序起始地址\n默认 0x7AE9",
        initialvalue=0x7AE9,
        minvalue=0x7AE9,
